...
class TrademarkAdmin(admin.ModelAdmin):
    list_display = ('user', 'name', 'status', 'get_user_plan', 'created_at', 'is_verified')
    list_filter = ('status', 'user__plan', 'is_verified', 'created_at')
    search_fields = ('user__username', 'user__email', 'name', 'status')
    list_editable = ('status',)

    def get_user_plan(self, obj):
        return obj.user.plan
    get_user_plan.short_description = 'Plan'
    get_user_plan.admin_order_field = 'user__plan'
